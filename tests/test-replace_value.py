...
def test_replace_value_returns_dataframe():
    result = replace_value(candy_df, 'candy', 'smarties', 'm&ms')
...
def test_replace_value_values():
    pd.testing.assert_frame_equal(replace_value(candy_df, 'candy', 'candy corn', 'dirt'),
                                  replace_str_output_candy_corn)
    pd.testing.assert_frame_equal(replace_value(candy_df, 'candy', 'kitkat', 'gold'),
                                  replace_str_output_kitkat)
    pd.testing.assert_frame_equal(replace_value(candy_df, 'rating', -5, -10),
                                  replace_int_output_negative)
    pd.testing.assert_frame_equal(replace_value(candy_df, 'rating', 0, 3),
                                  replace_int_output_zero)
    pd.testing.assert_frame_equal(replace_value(candy_df, 'rating', 5, 10),
                                  replace_int_output_positive)
    pd.testing.assert_frame_equal(replace_value(candy_df, 'rating', 80.0, 100.0),
